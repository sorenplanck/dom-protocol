...
1. Responder sends ephemeral key, encrypted static key, encrypted handshake confirmation
1. Initiator sends encrypted static key, encrypted handshake confirmation
...
1. The range proofs (each output value in [0, 2⁵²))
1. The Schnorr signatures on the kernels (proof of knowledge of the excess private key)
1. The cut-through invariants (no duplicate commitments, no orphaned inputs)
1. The coinbase rules (one and only one coinbase kernel per block; coinbase outputs locked for 1,000 blocks)
...
1. PoW (block hash meets compact target via RandomX)
1. Future timestamp bound (`t < now + 120s`)
1. Median time past (strict monotonic over 11-block window)
1. Previous block exists and is in the main chain
1. Height = prev.height + 1
1. Target matches ASERT calculation
1. Total difficulty matches prev + target_to_difficulty(target)
...
|**Range proofs**              |Bulletproofs (via grin secp256k1zkp, custom H_DOM generator)|RFC-0002         |
