...
static void bench_generator_generate(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(dominterop_secp_v0_10_0_generator_generate(data->ctx, &gen, data->key));
        data->key[i & 31]++;
    }
}
...
static void bench_generator_generate_blinded(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        dominterop_secp_v0_10_0_generator gen;
        CHECK(dominterop_secp_v0_10_0_generator_generate_blinded(data->ctx, &gen, data->key, data->blind));
        data->key[1 + (i & 30)]++;
        data->blind[1 + (i & 30)]++;
    }
}
