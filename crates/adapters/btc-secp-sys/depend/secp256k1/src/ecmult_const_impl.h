 ...
 */
static void dominterop_secp_v0_10_0_ecmult_const_odd_multiples_table_globalz(dominterop_secp_v0_10_0_ge *pre, dominterop_secp_v0_10_0_fe *globalz, const dominterop_secp_v0_10_0_gej *a) {
    dominterop_secp_v0_10_0_fe zr[ECMULT_CONST_TABLE_SIZE];

    dominterop_secp_v0_10_0_ecmult_odd_multiples_table(ECMULT_CONST_TABLE_SIZE, pre, zr, globalz, a);
    dominterop_secp_v0_10_0_ge_table_set_globalz(ECMULT_CONST_TABLE_SIZE, pre, zr);
}
...
#define ECMULT_CONST_TABLE_GET_GE(r,pre,n) do { \
    ...
    dominterop_secp_v0_10_0_fe neg_y; \
    ...
    for (m = 1; m < ECMULT_CONST_TABLE_SIZE; m++) { \
        ...
        dominterop_secp_v0_10_0_fe_cmov(&(r)->x, &(pre)[m].x, m == index); \
        dominterop_secp_v0_10_0_fe_cmov(&(r)->y, &(pre)[m].y, m == index); \
    } \
    (r)->infinity = 0; \
    dominterop_secp_v0_10_0_fe_negate(&neg_y, &(r)->y, 1); \
    dominterop_secp_v0_10_0_fe_cmov(&(r)->y, &neg_y, negative); \
} while(0)

/* For K as defined in the comment of dominterop_secp_v0_10_0_ecmult_const, we have several precomputed
 * formulas/constants.
 * - in exhaustive test mode, we give an explicit expression to compute it at compile time: */
#ifdef EXHAUSTIVE_TEST_ORDER
static const dominterop_secp_v0_10_0_scalar dominterop_secp_v0_10_0_ecmult_const_K = ((SECP256K1_SCALAR_CONST(0, 0, 0, (1U << (ECMULT_CONST_BITS - 128)) - 2U, 0, 0, 0, 0) + EXHAUSTIVE_TEST_ORDER - 1U) * (1U + EXHAUSTIVE_TEST_LAMBDA)) % EXHAUSTIVE_TEST_ORDER;
/* - for the real secp256k1 group we have constants for various ECMULT_CONST_BITS values. */
#elif ECMULT_CONST_BITS == 129
/* For GROUP_SIZE = 1,3. */
static const dominterop_secp_v0_10_0_scalar dominterop_secp_v0_10_0_ecmult_const_K = SECP256K1_SCALAR_CONST(0xac9c52b3ul, 0x3fa3cf1ful, 0x5ad9e3fdul, 0x77ed9ba4ul, 0xa880b9fcul, 0x8ec739c2ul, 0xe0cfc810ul, 0xb51283ceul);
#elif ECMULT_CONST_BITS == 130
/* For GROUP_SIZE = 2,5. */
static const dominterop_secp_v0_10_0_scalar dominterop_secp_v0_10_0_ecmult_const_K = SECP256K1_SCALAR_CONST(0xa4e88a7dul, 0xcb13034eul, 0xc2bdd6bful, 0x7c118d6bul, 0x589ae848ul, 0x26ba29e4ul, 0xb5c2c1dcul, 0xde9798d9ul);
#elif ECMULT_CONST_BITS == 132
/* For GROUP_SIZE = 4,6 */
static const dominterop_secp_v0_10_0_scalar dominterop_secp_v0_10_0_ecmult_const_K = SECP256K1_SCALAR_CONST(0x76b1d93dul, 0x0fae3c6bul, 0x3215874bul, 0x94e93813ul, 0x7937fe0dul, 0xb66bcaaful, 0xb3749ca5ul, 0xd7b6171bul);
#else
...
static void dominterop_secp_v0_10_0_ecmult_const(dominterop_secp_v0_10_0_gej *r, const dominterop_secp_v0_10_0_ge *a, const dominterop_secp_v0_10_0_scalar *q) {
     ...
     * dominterop_secp_v0_10_0_scalar_split_lambda function which performs such a split with the resulting s1
    ...
    static const dominterop_secp_v0_10_0_scalar S_OFFSET = SECP256K1_SCALAR_CONST(0, 0, 0, 1, 0, 0, 0, 0);
    dominterop_secp_v0_10_0_scalar s, v1, v2;
    dominterop_secp_v0_10_0_ge pre_a[ECMULT_CONST_TABLE_SIZE];
    dominterop_secp_v0_10_0_ge pre_a_lam[ECMULT_CONST_TABLE_SIZE];
    dominterop_secp_v0_10_0_fe global_z;
    int group, i;

    /* We're allowed to be non-constant time in the point, and the code below (in particular,
     * dominterop_secp_v0_10_0_ecmult_const_odd_multiples_table_globalz) cannot deal with infinity in a
     * constant-time manner anyway. */
    if (dominterop_secp_v0_10_0_ge_is_infinity(a)) {
        dominterop_secp_v0_10_0_gej_set_infinity(r);
        return;
    }

    /* Compute v1 and v2. */
    dominterop_secp_v0_10_0_scalar_add(&s, q, &dominterop_secp_v0_10_0_ecmult_const_K);
    dominterop_secp_v0_10_0_scalar_half(&s, &s);
    dominterop_secp_v0_10_0_scalar_split_lambda(&v1, &v2, &s);
    dominterop_secp_v0_10_0_scalar_add(&v1, &v1, &S_OFFSET);
    dominterop_secp_v0_10_0_scalar_add(&v2, &v2, &S_OFFSET);

#ifdef VERIFY
    /* Verify that v1 and v2 are in range [0, 2^129-1]. */
    for (i = 129; i < 256; ++i) {
        VERIFY_CHECK(dominterop_secp_v0_10_0_scalar_get_bits(&v1, i, 1) == 0);
        VERIFY_CHECK(dominterop_secp_v0_10_0_scalar_get_bits(&v2, i, 1) == 0);
    }
#endif
     ...
     */
    dominterop_secp_v0_10_0_gej_set_ge(r, a);
    dominterop_secp_v0_10_0_ecmult_const_odd_multiples_table_globalz(pre_a, &global_z, r);
    for (i = 0; i < ECMULT_CONST_TABLE_SIZE; i++) {
        dominterop_secp_v0_10_0_ge_mul_lambda(&pre_a_lam[i], &pre_a[i]);
    }
    ...
    for (group = ECMULT_CONST_GROUPS - 1; group >= 0; --group) {
        ...
        unsigned int bits1 = dominterop_secp_v0_10_0_scalar_get_bits_var(&v1, group * ECMULT_CONST_GROUP_SIZE, ECMULT_CONST_GROUP_SIZE);
        unsigned int bits2 = dominterop_secp_v0_10_0_scalar_get_bits_var(&v2, group * ECMULT_CONST_GROUP_SIZE, ECMULT_CONST_GROUP_SIZE);
        dominterop_secp_v0_10_0_ge t;
        int j;

        ECMULT_CONST_TABLE_GET_GE(&t, pre_a, bits1);
        if (group == ECMULT_CONST_GROUPS - 1) {
            /* Directly set r in the first iteration. */
            dominterop_secp_v0_10_0_gej_set_ge(r, &t);
        } else {
            /* Shift the result so far up. */
            for (j = 0; j < ECMULT_CONST_GROUP_SIZE; ++j) {
                dominterop_secp_v0_10_0_gej_double(r, r);
            }
            dominterop_secp_v0_10_0_gej_add_ge(r, r, &t);
        }
    ...
    dominterop_secp_v0_10_0_fe_mul(&r->z, &r->z, &global_z);
}

static int dominterop_secp_v0_10_0_ecmult_const_xonly(dominterop_secp_v0_10_0_fe* r, const dominterop_secp_v0_10_0_fe *n, const dominterop_secp_v0_10_0_fe *d, const dominterop_secp_v0_10_0_scalar *q, int known_on_curve) {
    ...
    dominterop_secp_v0_10_0_fe g, i;
    dominterop_secp_v0_10_0_ge p;
    dominterop_secp_v0_10_0_gej rj;

    /* Compute g = (n^3 + B*d^3). */
    dominterop_secp_v0_10_0_fe_sqr(&g, n);
    dominterop_secp_v0_10_0_fe_mul(&g, &g, n);
    if (d) {
        dominterop_secp_v0_10_0_fe b;
        VERIFY_CHECK(!dominterop_secp_v0_10_0_fe_normalizes_to_zero(d));
        dominterop_secp_v0_10_0_fe_sqr(&b, d);
        VERIFY_CHECK(SECP256K1_B <= 8); /* magnitude of b will be <= 8 after the next call */
        dominterop_secp_v0_10_0_fe_mul_int(&b, SECP256K1_B);
        dominterop_secp_v0_10_0_fe_mul(&b, &b, d);
        dominterop_secp_v0_10_0_fe_add(&g, &b);
        if (!known_on_curve) {
             ...
             * <=> is_square(g * d)
             */
            dominterop_secp_v0_10_0_fe c;
            dominterop_secp_v0_10_0_fe_mul(&c, &g, d);
            if (!dominterop_secp_v0_10_0_fe_is_square_var(&c)) return 0;
        }
    } else {
        dominterop_secp_v0_10_0_fe_add_int(&g, SECP256K1_B);
        if (!known_on_curve) {
            /* g at this point equals x^3 + 7. Test if it is square. */
            if (!dominterop_secp_v0_10_0_fe_is_square_var(&g)) return 0;
        }
    }

    /* Compute base point P = (n*g, g^2), the effective affine version of (n*g, g^2, v), which has
     * corresponding affine X coordinate n/d. */
    dominterop_secp_v0_10_0_fe_mul(&p.x, &g, n);
    dominterop_secp_v0_10_0_fe_sqr(&p.y, &g);
    p.infinity = 0;

    /* Perform x-only EC multiplication of P with q. */
    VERIFY_CHECK(!dominterop_secp_v0_10_0_scalar_is_zero(q));
    dominterop_secp_v0_10_0_ecmult_const(&rj, &p, q);
    VERIFY_CHECK(!dominterop_secp_v0_10_0_gej_is_infinity(&rj));
     ...
     * (X / (Z^2*d*g)). */
    dominterop_secp_v0_10_0_fe_sqr(&i, &rj.z);
    dominterop_secp_v0_10_0_fe_mul(&i, &i, &g);
    if (d) dominterop_secp_v0_10_0_fe_mul(&i, &i, d);
    dominterop_secp_v0_10_0_fe_inv(&i, &i);
    dominterop_secp_v0_10_0_fe_mul(r, &rj.x, &i);
