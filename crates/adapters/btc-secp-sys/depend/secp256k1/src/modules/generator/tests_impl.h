...
static void test_generator_api(void) {
    ...
    dominterop_secp_v0_10_0_generator gen;

    dominterop_secp_v0_10_0_testrand256(key);
    dominterop_secp_v0_10_0_testrand256(blind);

    CHECK(dominterop_secp_v0_10_0_generator_generate(CTX, &gen, key) == 1);
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_generator_generate(CTX, NULL, key));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_generator_generate(CTX, &gen, NULL));

    CHECK(dominterop_secp_v0_10_0_generator_generate_blinded(CTX, &gen, key, blind) == 1);
    CHECK_ILLEGAL(STATIC_CTX, dominterop_secp_v0_10_0_generator_generate_blinded(STATIC_CTX, &gen, key, blind));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_generator_generate_blinded(CTX, NULL, key, blind));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_generator_generate_blinded(CTX, &gen, NULL, blind));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_generator_generate_blinded(CTX, &gen, key, NULL));

    CHECK(dominterop_secp_v0_10_0_generator_serialize(CTX, sergen, &gen) == 1);
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_generator_serialize(CTX, NULL, &gen));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_generator_serialize(CTX, sergen, NULL));

    CHECK(dominterop_secp_v0_10_0_generator_serialize(CTX, sergen, &gen) == 1);
    CHECK(dominterop_secp_v0_10_0_generator_parse(CTX, &gen, sergen) == 1);
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_generator_parse(CTX, NULL, sergen));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_generator_parse(CTX, &gen, NULL));
}

static void test_shallue_van_de_woestijne(void) {
    /* Matches with the output of the shallue_van_de_woestijne.sage SAGE program */
    static const dominterop_secp_v0_10_0_ge_storage results[34] = {
    ...
    dominterop_secp_v0_10_0_ge_storage ges;
    int i, s;
    for (i = 0; i <= 16; i++) {
        dominterop_secp_v0_10_0_fe_set_int(&fe, i);

        for (s = 0; s < 2; s++) {
            if (s) {
                dominterop_secp_v0_10_0_fe_negate(&fe, &fe, 1);
                dominterop_secp_v0_10_0_fe_normalize(&fe);
            }
            shallue_van_de_woestijne(&ge, &fe);
            CHECK(dominterop_secp_v0_10_0_ge_is_valid_var(&ge));
            dominterop_secp_v0_10_0_ge_to_storage(&ges, &ge);
            CHECK(dominterop_secp_v0_10_0_memcmp_var(&ges, &results[i * 2 + s], sizeof(dominterop_secp_v0_10_0_ge_storage)) == 0);
        }
    }
}

static void test_generator_generate(void) {
    static const dominterop_secp_v0_10_0_ge_storage results[32] = {
    ...
    dominterop_secp_v0_10_0_scalar_set_b32(&sc, s, NULL);
    for (i = 1; i <= 32; i++) {
        memset(v, 0, 31);
        v[31] = i;
        CHECK(dominterop_secp_v0_10_0_generator_generate_blinded(CTX, &gen, v, s));
        dominterop_secp_v0_10_0_generator_load(&ge, &gen);
        dominterop_secp_v0_10_0_ge_to_storage(&ges, &ge);
        CHECK(dominterop_secp_v0_10_0_memcmp_var(&ges, &results[i - 1], sizeof(dominterop_secp_v0_10_0_ge_storage)) == 0);
        CHECK(dominterop_secp_v0_10_0_generator_generate(CTX, &gen, v));
        dominterop_secp_v0_10_0_generator_load(&ge, &gen);
        dominterop_secp_v0_10_0_ge_to_storage(&ges, &ge);
        CHECK(dominterop_secp_v0_10_0_memcmp_var(&ges, &results[i - 1], sizeof(dominterop_secp_v0_10_0_ge_storage)) == 0);
    }
...
static void test_generator_fixed_vector(void) {
    ...
    dominterop_secp_v0_10_0_generator parse;

    CHECK(dominterop_secp_v0_10_0_generator_parse(CTX, &parse, two_g));
    CHECK(dominterop_secp_v0_10_0_generator_serialize(CTX, result, &parse));
    CHECK(dominterop_secp_v0_10_0_memcmp_var(two_g, result, 33) == 0);

    result[0] = 0x0a;
    CHECK(dominterop_secp_v0_10_0_generator_parse(CTX, &parse, result));
    result[0] = 0x08;
    CHECK(!dominterop_secp_v0_10_0_generator_parse(CTX, &parse, result));
}

static void test_pedersen_api(void) {
    dominterop_secp_v0_10_0_pedersen_commitment commit;
    const dominterop_secp_v0_10_0_pedersen_commitment *commit_ptr = &commit;
    unsigned char blind[32];
    unsigned char blind_out[32];
    const unsigned char *blind_ptr = blind;
    unsigned char *blind_out_ptr = blind_out;
    uint64_t val = dominterop_secp_v0_10_0_testrand32();

    dominterop_secp_v0_10_0_testrand256(blind);
    CHECK(dominterop_secp_v0_10_0_pedersen_commit(CTX, &commit, blind, val, dominterop_secp_v0_10_0_generator_h) != 0);
    CHECK_ILLEGAL(STATIC_CTX, dominterop_secp_v0_10_0_pedersen_commit(STATIC_CTX, &commit, blind, val, dominterop_secp_v0_10_0_generator_h));

    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_commit(CTX, NULL, blind, val, dominterop_secp_v0_10_0_generator_h));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_commit(CTX, &commit, NULL, val, dominterop_secp_v0_10_0_generator_h));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_commit(CTX, &commit, blind, val, NULL));

    CHECK(dominterop_secp_v0_10_0_pedersen_blind_sum(CTX, blind_out, &blind_ptr, 1, 1) != 0);
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_blind_sum(CTX, NULL, &blind_ptr, 1, 1));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_blind_sum(CTX, blind_out, NULL, 1, 1));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_blind_sum(CTX, blind_out, &blind_ptr, 0, 1));
    CHECK(dominterop_secp_v0_10_0_pedersen_blind_sum(CTX, blind_out, &blind_ptr, 0, 0) != 0);

    CHECK(dominterop_secp_v0_10_0_pedersen_commit(CTX, &commit, blind, val, dominterop_secp_v0_10_0_generator_h) != 0);
    CHECK(dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, &commit_ptr, 1, &commit_ptr, 1) != 0);
    CHECK(dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, NULL, 0, &commit_ptr, 1) == 0);
    CHECK(dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, &commit_ptr, 1, NULL, 0) == 0);
    CHECK(dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, NULL, 0, NULL, 0) != 0);
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, NULL, 1, &commit_ptr, 1));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, &commit_ptr, 1, NULL, 1));

    CHECK(dominterop_secp_v0_10_0_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 1, 0) != 0);
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 1, 1));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 0, 0));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_blind_generator_blind_sum(CTX, NULL, &blind_ptr, &blind_out_ptr, 1, 0));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_blind_generator_blind_sum(CTX, &val, NULL, &blind_out_ptr, 1, 0));
    CHECK_ILLEGAL(CTX, dominterop_secp_v0_10_0_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, NULL, 1, 0));
}

static void test_pedersen(void) {
    dominterop_secp_v0_10_0_pedersen_commitment commits[19];
    const dominterop_secp_v0_10_0_pedersen_commitment *cptr[19];
    ...
    int outputs;
    int total;
    inputs = (dominterop_secp_v0_10_0_testrand32() & 7) + 1;
    outputs = (dominterop_secp_v0_10_0_testrand32() & 7) + 2;
    ...
    totalv = 0;
    for (i = 0; i < inputs; i++) {
        values[i] = dominterop_secp_v0_10_0_testrandi64(0, INT64_MAX - totalv);
        totalv += values[i];
    }
    for (i = 0; i < outputs - 1; i++) {
        values[i + inputs] = dominterop_secp_v0_10_0_testrandi64(0, totalv);
    ...
    for (i = 0; i < total - 1; i++) {
        ...
        dominterop_secp_v0_10_0_scalar_get_b32(&blinds[i * 32], &s);
    }
    CHECK(dominterop_secp_v0_10_0_pedersen_blind_sum(CTX, &blinds[(total - 1) * 32], bptr, total - 1, inputs));
    for (i = 0; i < total; i++) {
        unsigned char result[33];
        dominterop_secp_v0_10_0_pedersen_commitment parse;

        CHECK(dominterop_secp_v0_10_0_pedersen_commit(CTX, &commits[i], &blinds[i * 32], values[i], dominterop_secp_v0_10_0_generator_h));
        CHECK(dominterop_secp_v0_10_0_pedersen_commitment_serialize(CTX, result, &commits[i]));
        CHECK(dominterop_secp_v0_10_0_pedersen_commitment_parse(CTX, &parse, result));
        CHECK(dominterop_secp_v0_10_0_memcmp_var(&commits[i], &parse, 33) == 0);
    }
    CHECK(dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, cptr, inputs, &cptr[inputs], outputs));
    CHECK(dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, &cptr[inputs], outputs, cptr, inputs));
    if (inputs > 0 && values[0] > 0) {
        CHECK(!dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, cptr, inputs - 1, &cptr[inputs], outputs));
    }
    random_scalar_order(&s);
    for (i = 0; i < 4; i++) {
        dominterop_secp_v0_10_0_scalar_get_b32(&blinds[i * 32], &s);
    ...
    CHECK(dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, &cptr[0], 1, &cptr[0], 1));
    CHECK(dominterop_secp_v0_10_0_pedersen_verify_tally(CTX, &cptr[1], 1, &cptr[1], 1));
}
...
static void test_pedersen_commitment_fixed_vector(void) {
    ...
    CHECK(dominterop_secp_v0_10_0_pedersen_commitment_parse(CTX, &parse, two_g));
    CHECK(dominterop_secp_v0_10_0_pedersen_commitment_serialize(CTX, result, &parse));
    CHECK(dominterop_secp_v0_10_0_memcmp_var(two_g, result, 33) == 0);
