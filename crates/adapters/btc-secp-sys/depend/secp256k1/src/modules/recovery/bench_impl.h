...
static void bench_recover(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(dominterop_secp_v0_10_0_ecdsa_recoverable_signature_parse_compact(data->ctx, &sig, data->sig, i % 2));
        CHECK(dominterop_secp_v0_10_0_ecdsa_recover(data->ctx, &pubkey, &sig, data->msg));
        CHECK(dominterop_secp_v0_10_0_ec_pubkey_serialize(data->ctx, pubkeyc, &pubkeylen, &pubkey, SECP256K1_EC_COMPRESSED));
        for (j = 0; j < 32; j++) {
