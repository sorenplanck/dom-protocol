//! Current-tree comparison adapter for independently frozen Phase 1 vectors.
//!
//! PUBLIC/INSECURE TEST-VECTOR WARNING: the referenced fixtures contain fixed
//! signing shares, auxiliary randomness, nonce material, and adaptor secrets.
//! They are public test evidence only, must never be used with real funds, and
//! must never be imported into a production key or nonce lifecycle.
...
impl Checks {
    fn bytes(&mut self, name: &str, actual: &[u8], expected_hex: &str) {
        ...
        if actual != expected {
            panic!("independent vector mismatch at {name}");
        }
        self.count += 1;
    }
    ...
    fn truth(&mut self, name: &str, value: bool) {
        if !value {
            panic!("independent boolean mismatch at {name}");
        }
        self.count += 1;
    }
...
fn secret(value: &str) -> SigningShareV1 {
    SigningShareV1::from_be_bytes(array(value)).expect("frozen scalar must parse")
}
...
fn compare_case(checks: &mut Checks, root: &Value, case: &Value) {
    ...
    for (index, participant) in participant_values.iter().enumerate() {
        ...
        let signing_key = signing_share.public_key().clone();
        checks.bytes(
        ...
        let expected_1 = array::<32>(text(kdf, "k1_be32"));
        let expected_2 = array::<32>(text(kdf, "k2_be32"));
        checks.truth(
            &format!("{participant_name}.scalar_from_wide.k1"),
            reduced_1.as_ref() == expected_1,
        );
        checks.truth(
            &format!("{participant_name}.scalar_from_wide.k2"),
            reduced_2.as_ref() == expected_2,
        );
        ...
        let (first_public, second_public) = derived_pair
            .public_keys()
            .expect("frozen public nonce pair");
        checks.bytes(
    ...
    if purpose == PurposeV1::ClaimAdaptor {
        ...
        checks.bytes(
            ...
            &extracted
                .public_point()
                .expect("extracted adaptor point")
                .to_compressed_bytes(),
...
pub(crate) fn run(vector_path: &Path) -> usize {
    let document: Value =
