...
//! `range_proof_verify` runs on the attacker-controlled 33-byte commitment.
//! This target drives the bridge through that public surface with a fully
//! attacker-controlled commitment prefix+X:
//!
...
use dom_crypto::range_proof_verify;
...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = range_proof_verify(&commitment, proof);
});
