...
pub const PRODUCTION_KNOWN_LIMITS_V1: &[&str] = &[
    ...
    "Level1BlindedRoutes: the Level-1 blinded-route family (per-leg lock points, DR-PRIV-001) is admitted through `admit_validated_composed_route_v3` but the production composite loop still composes on the V2 shared-point family until the V3 promotion is ratified; and Level 1 claims EXACTLY T0-unlinkability of the cryptographic artifacts — amounts and timing still correlate (denomination/batching policy is not part of Level 1), the solver links the legs by construction (blinding the solver is the separately gated Level 2), and the leg offset is as sensitive as every other route secret",
];
...
pub fn run_production_v1(options: &ProductionRunOptionsV1) -> Result<(), ProductionRunErrorV1> {
    ...
    let child_router = ProductionSettlementChildRouterV1::new(
        dom_child,
        Some(ProductionSettlementChildRouterV1::authenticate_evm(
            evm_child,
        )),
    ...
    .map_err(|_| ProductionRunErrorV1::PlanSource)?;
    let plan_persistence =
        ProductionSettlementPlanPersistenceOwnerV1::new(plan_authority, trusted_now_millis_v1()?)
            .map_err(|_| ProductionRunErrorV1::PlanSource)?;
