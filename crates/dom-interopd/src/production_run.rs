...
pub const MISSING_PRODUCTION_PARTS_V1: &[&str] = &[
    ...
    "PublicSecretSource (upstream reveal extraction): `ProductionPublicSecretSourceRouterV1` covers DOM, EVM and Bitcoin. A route whose UPSTREAM counterparty is Solana or Monero has no production secret source to extract the revealed 252-bit scalar from the counterparty's on-chain reveal (Solana Claim `revealed_secret_be`, or the Monero sweep). This is per-chain extraction work, not composition glue, and is required before a SOL/XMR-upstream route composes. An EVM/BTC-counterparty route needs none of it.",
