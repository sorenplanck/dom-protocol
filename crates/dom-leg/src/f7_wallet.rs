...
impl F7DomDsc1CheckpointV1 {
    fn new(purpose: PurposeV1, durable_prefix_len: usize) -> Result<Self, F7WalletCompositorError> {
        ...
        let purpose = match purpose {
            ...
            PurposeV1::Sponsor => return Err(F7WalletCompositorError::Binding),
        };
        Ok(Self {
            purpose,
            durable_prefix_len,
...
impl F7ProductionContractsCompositionV1 {
    ...
    pub fn create_production(
        ...
        require_production_policy(&policy)?;
        let session_store =
            ContractsSessionStoreV1::create_production(parent, session_store_root, policy)?;
        Self::from_parts(
            session_store,
            participants,
            network_magic,
    ...
    pub fn open_production(
        ...
        require_production_policy(&policy)?;
        let session_store =
            ContractsSessionStoreV1::open_production(parent, session_store_root, policy)?;
        Self::from_parts(
            session_store,
            participants,
            network_magic,
    ...
    fn from_parts(
        ...
        require_production_policy(session_store.policy())?;
...
impl F7VerifiedDomClaimArtifactV1 {
    ...
    /// Consumes the typed authority into the exact canonical public bytes.
    #[must_use]
    pub fn into_canonical_bytes(self) -> Vec<u8> {
        self.verified.canonical_bytes().to_vec()
    }
}
...
impl F7WalletPreSignedDomClaimV1 {
    ...
    ) -> Result<(F7VerifiedDomClaimArtifactV1, F7WalletDomClaimExtractionV1), F7WalletCompositorError>
    {
        ...
        let secret = AdaptorSecret::from_be_bytes(revealed.0)?;
