...
fn single_subnet_flood_monopolises_connectable_set() {
    ...
    // Attacker floods max_peers addresses, all from 10.0.0.0/24 (one subnet).
    ...
    for i in 0..32u8 {
        pex.add_peer(format!("198.51.{}.7:8333", i));
    }
...
proptest! {
    ...
    fn no_subnet_cap_bounds_attacker_share(
        ...
        // Attacker fills first, all from 172.16.0.0/24.
        for i in 0..cap {
            pex.add_peer(format!("172.16.0.{}:8333", i % 256));
        }
        // Honest addrs from many distinct subnets arrive after.
        for i in 0..honest {
            pex.add_peer(format!("203.0.{}.{}:8333", (i / 256) % 256, i % 256));
        }
        let connectable = pex.connectable_peers();
        let attacker = connectable
            .iter()
            .filter(|p| p.addr.starts_with("172.16.0."))
            .count();
