...
> **SUPERSEDED (2026-08-10).** This v0.3 was replaced by **v0.4** (`DOM-Interop-Foundation-Document-v0.4.md`), the first English-normative version, and is out of agent circulation (§12.2). Kept as history only; nothing here is context authority.
...
Estado:              PARCIALMENTE RATIFICADO — D-000, D-001, D-005, D-006 e
                     A7 ratificados (ordem /goat e decisões do operador de
                     2026-08-09, registradas na seção 12); o restante segue
                     RASCUNHO até ratificação
Nome do produto:     "DOM Interop" é nome de trabalho [PROPOSTA] — A1 mantida
                     aberta por decisão do operador (2026-08-09)
Substitui:           v0.2.1, v0.2, v0.1 e o "KAYSTRA-USPE-KEYSTONE-DOCUMENTO-
                     MESTRE v1.0.1" (todos SUPERSEDED como autoridade de
                     contexto; blocos úteis portados para cá)
Estado da F1:        docs/reports/F1-CLOSURE.md (G-F1 = NOT_PASS; bloqueio
                     P0 na §1.3 daquele relatório)
```
...
**Atualização v0.2.1 (D-002):** a implementação de referência no
repositório (`contracts/src/ConditionLockV2.sol` do esqueleto) substitui o
push-pay que reverte por push com fallback pull: se o destinatário rejeitar
ETH, o valor vira crédito em `pendingWithdrawals` sacável por `withdraw()`,
e o settlement — incluindo a emissão de `Claimed(t)` — nunca é bloqueado
pelo payout. O esqueleto é a versão canônica; o listing acima permanece
como explicação didática do mecanismo.
...
dom-leg             perna DOM: sessão, roster, transcript, 2-de-2,
                    verify/adapt/extract (importa o pin)
dom-vault           NonceVaultV1 durável: reservas, permits, sealing,
                    recovery, resend byte-idêntico (importa o pin) [D-005]
...
store               persistência local autoritativa NEUTRA (SQLite/WAL,
                    ADR-A7): journal, idempotência, CAS, cursores, outbox —
                    não conhece dom-adaptor, nonces nem segredos
```
...
Regra de dependência (grep-gate em CI, atualizada por D-005): apenas
`crates/dom-leg` e `crates/dom-vault` podem conter `use dom_adaptor` /
`dom-adaptor` no Cargo.toml. `kaystra-core` e `uspe` importam somente
`dom-leg` e `counterparty-api`. Errata registrada: `dom-adaptor` NÃO
reexporta `SchnorrSignature`, `PublicKey` nem `Transaction`; nomear esses
tipos exige `dom-crypto` e `dom-consensus` pinados no MESMO rev — os três
pins são imutáveis em conjunto (§9.2).
...
### 4.7 store — persistência — [DECIDIDO; A7 resolvida: SQLite/WAL, ADR-A7]
...
**Atualização v0.2.1 (D-003, D-004):** a implementação de referência no
repositório (`crates/kaystra-core/src/state.rs`) evolui este listing:
(a) contexto `SettlementContext` com `last_observed_height` como chave de
idempotência de reorg (reentrega at-least-once do mesmo evento é inócua);
(b) novo evento `FundingAbsent` (`Confirming → ReadyToFund`) para re-arme
antecipado quando a revalidação pós-reorg conclui que o funding sumiu;
(c) reobservação `FundingObserved` aceita em `Confirming` (refresh de
altura, idempotente); (d) decisão registrada em código: `RefundConfirmed`
é aceito sem `TimelockExpired` prévio porque o timelock é autoridade da
CHAIN — a máquina registra a realidade observada. O esqueleto é a versão
canônica; o listing acima permanece como explicação didática.
...
```text
A1  Nome do produto — ABERTA por decisão do operador (2026-08-09):
    "DOM Interop" segue como nome de trabalho.
A2  Licença do produto; relicenciamento/reescrita do Keystone (BUSL) —
    ABERTA; medida provisória ratificada (D-007): proprietária por ora
    (LICENSE "all rights reserved", SPDX UNLICENSED, publish = false).
...
A7  RESOLVIDA — SQLite/WAL (docs/adr/ADR-A7-SQLite-WAL.md, ratificado
    2026-08-06); alocação do adaptador ajustada por D-005.
...
```text
D-000  2026-08-05  RATIFICADO (ordem /goat do operador, 2026-08-09)
  Problema:      inexistência de autoridade única de contexto e escopo.
  Decisão:       ratificar a cápsula P.3, itens 1–10 (projeto único;
                 desenvolvimento separado da DOM; topologia DOM-cêntrica
                 DOM↔X; destino DOM v2 acima do consenso; pin 180b731;
                 autocustódia; antipoder; DL2P/CIPHER/Lend fora; mocks
                 nunca satisfazem gate).
  Alternativas rejeitadas: três sistemas multichain independentes com DOM
                 como perfil opcional (enquadramento do v1.0.1).
  Impacto:       todo o documento e todo o repositório.
  Substitui:     —
  Componentes:   todos.

D-001  2026-08-05  RATIFICADO (ordem /goat do operador, 2026-08-09)
  Problema:      dois documentos mestres concorrentes restauram contextos
                 contraditórios em conversas com agentes.
  Decisão:       "KAYSTRA-USPE-KEYSTONE-DOCUMENTO-MESTRE v1.0.1" recebe a
                 marca SUPERSEDED na primeira página e sai de circulação
                 de agentes; blocos úteis já portados para cá (§P.2, §3.4,
                 §4.6, §12.2).
  Alternativas rejeitadas: manter os dois em paralelo.
  Impacto:       governança documental.
  Substitui:     v1.0.1 como autoridade de contexto.
  Componentes:   docs/normative.

D-002  2026-08-05  RATIFICAÇÃO PENDENTE
  Problema:      push-pay que reverte prende fundos no refund quando o
                 funder é um contrato que rejeita ETH (claim já expirado
                 => travamento permanente); achado da auditoria hostil do
                 esqueleto.
  Decisão:       payout com fallback pull em claim e refund
                 (pendingWithdrawals + withdraw()); settlement e emissão
                 de Claimed(t) nunca dependem do payout.
  Alternativas rejeitadas: push puro (original, herdado do ConditionVM);
                 pull puro (pior UX no caminho feliz).
  Impacto:       contrato da perna EVM; suíte Foundry da F3 deve cobrir
                 destinatário que reverte, PayoutDeferred e withdraw.
  Substitui:     listing original da §2.3 (v0.2).
  Componentes:   contracts/src/ConditionLockV2.sol.

D-003  2026-08-05  RATIFICAÇÃO PENDENTE
  Problema:      (a) reentrega at-least-once de ReorgInvalidated regredia
                 o settlement duas vezes (bug real pego por teste de
                 crash); (b) após regressão com funding desaparecido, o
                 settlement ficava parado até o timelock, sem re-arme.
  Decisão:       SettlementContext.last_observed_height como chave de
                 idempotência de reorg; novo evento FundingAbsent
                 (Confirming → ReadyToFund, ilegal nos demais estados);
                 FundingObserved aceito em Confirming como reobservação
                 idempotente com refresh de altura.
  Alternativas rejeitadas: afrouxar o teste de reentrega; regressão
                 iterativa multi-passo (adiada para a tabela completa F2).
  Impacto:       máquina de estados §6; motor F2.
  Substitui:     listing original da §6 (v0.2).
  Componentes:   crates/kaystra-core/src/state.rs.

D-004  2026-08-05  RATIFICAÇÃO PENDENTE
  Problema:      RefundConfirmed aceito sem TimelockExpired prévio parece
                 bug e convida "conserto" errado.
  Decisão:       é design: o timelock é autoridade da CHAIN (deadline no
                 contrato EVM, CSV no BTC, refund pré-assinado na DOM); a
                 máquina registra a realidade observada. Checagem
                 duplicada na máquina criaria segunda autoridade sobre o
                 timelock e risco de divergência. Documentado em
                 comentário no código.
  Alternativas rejeitadas: exigir TimelockExpired antes de aceitar
                 RefundConfirmed.
  Impacto:       semântica da §6; revisões futuras.
  Substitui:     —
  Componentes:   crates/kaystra-core/src/state.rs.

D-005  2026-08-09  RATIFICADO (ordem /goat do operador)
  Problema:      o contrato NonceVaultV1 é um trait do dom-adaptor, mas a
                 fronteira §4.2 original só permitia ao dom-leg importar o
                 pin — e o store deve permanecer neutro (§4.7).
  Decisão:       crate próprio `dom-vault` para a implementação durável do
                 NonceVaultV1; a guarda de fronteira passa a permitir
                 dom-leg E dom-vault; o store continua sem tipos DOM.
  Alternativas rejeitadas: vault dentro do dom-leg (alocação do ADR-A7,
                 anterior à ordem); store importando o pin.
  Impacto:       §4.1, §4.2, guardas de CI.
  Substitui:     regra de crate único da §4.2 (v0.2/v0.2.1).
  Componentes:   crates/dom-vault, scripts/guards.sh.

D-006  2026-08-09  SATISFEITA PELO PIN (verificação registrada)
  Problema:      exigência de que PreparedExposureV1 não fosse forjável
                 por bytes crus.
  Decisão:       verificado que o pin 180b731 já satisfaz: construtores
                 pub(crate), campos privados, verificação interna do
                 permit ("Constructors are crate-private and no raw-byte
                 constructor exists"). Nenhuma mudança de pin.
  Componentes:   nenhum (evidência em docs/reports/F1-CLOSURE.md §1).

D-007  2026-08-09  RATIFICADO (decisão do operador, registrada em chat)
  Problema:      workspace e contratos com licença "TBD-A2" — estado
                 juridicamente indefinido.
  Decisão:       medida PROVISÓRIA: proprietária ("all rights reserved",
                 arquivo LICENSE; SPDX UNLICENSED nos contratos;
                 license-file no workspace; publish = false). A2 segue
                 ABERTA para a licença definitiva e o Keystone/BUSL.
  Alternativas rejeitadas: MIT OR Apache-2.0; Apache-2.0; GPL-3.0 (adiadas,
                 não descartadas).
  Impacto:       LICENSE, Cargo.toml (workspace + crates), contracts/src.
  Substitui:     —
  Componentes:   raiz do repositório.

R-001  2026-08-09  REGISTRO (não é decisão nova)
  G-F0 = DISPENSA PARA F1 (ordem /goat): A1, A2 e A12 seguem abertas; a
  dispensa permite fechar tecnicamente a F1 e NÃO autoriza F3.
  BLOQUEIO P0 da F1: o contrato NonceVaultV1 não é dirigível de fora do
  pin (todo tipo de entrada tem construtor pub(crate)); ação externa
  mínima: commit público do dom-adaptor expondo ponto de entrada de
  produção para a autoridade de sessão. Evidência e tabela método-a-método
  em docs/reports/F1-CLOSURE.md §1.2–1.3.
```
...
*Esta v0.2.1 substitui a v0.1 e o "KAYSTRA-USPE-KEYSTONE-DOCUMENTO-MESTRE
