...
> `DOM-Interop-Foundation-Document-v0.12.md` on 2026-08-10 (D-020: the
> addressed-flow sequence domain and the recipient-distinguishing
> idempotency key; D-021 and D-022: the registry entries for the
> already-ratified AD-1.2 and AD-1.4). It remains as history only and
> must not be used as context authority.
...
State:               PARTIALLY RATIFIED — D-000, D-001, D-005, D-006, D-007,
                     ...
                     D-016, D-017, D-018, D-019 and A7 ratified; the rest remains
                     DRAFT (/goat order and operator decisions of
                     2026-08-09/10, recorded in section 12)
Product name:        DECIDED (D-009): no standalone product name. This is
                     ...
                     component name
Supersedes:          v0.10, v0.9, v0.8, v0.7, v0.6, v0.5, v0.4, v0.3, v0.2.1, v0.2, v0.1 and the
...
Gate status:         G-F0 = PASS (docs/reports/F0-CLOSURE.md, waiver
                     ...
                     (docs/reports/F2-CLOSURE.md). G-F3 and G-F4:
                     implementation and adversarial suites complete,
                     Sepolia evidence runs in flight; NOT PASS until the
                     evidence lands and the operator ratifies. F5 IN
                     PROGRESS (D-012); execution spec is Annex M v3.2;
                     G-F5 NOT RUN (M.15.2 legs outstanding).
```
...
D-019  2026-08-10  RATIFIED (explicit operator decision, 2026-08-10)
  ...
  Executor note (reported, NOT ratified): three items of the decision
                 could not be implemented as written on the ratified V1
                 wire, and were implemented in the form the ratified
                 wire supports, with the divergence reported to the
                 operator rather than resolved by inference:
                 (a) the decision names `service`, `settlement_id`,
                     `message_id` and `payload_codec` among the fields
                     the authenticated digest must cover; none of the
                     four exists in the RelayEnvelopeV1 wire ratified by
                     D-018 (F6 spec §5.2). Adding them is a wire change
                     and, by D-018's own adoption terms, requires a new
                     normative version and express ratification. The
                     digest today covers every field §5.2 lists.
                 (b) test 12 as written requires a per-recipient
                     `message_id`/`sequence` space, which without
                     `message_id` would collide on the §6.1 key the same
                     decision forbids colliding. It is implemented in
                     the only non-colliding form the V1 wire allows: one
                     addressed envelope per recipient, each at its own
                     sequence in the sender's single session space.
                 (c) "the role obtained from the roster only after
                     authenticating the envelope" reads two ways against
                     the ratified §5.4, which puts the role at step 6
                     and the signature at step 7. The implementation
                     takes the role from the ROSTER and never from the
                     envelope's claim, and delivers the payload only
                     after step 7 — so no unauthenticated message can
                     act on a role — while keeping the ratified step
                     order intact.
...
*This v0.11 supersedes v0.10, v0.9, v0.8, v0.7, v0.6, v0.5, v0.4, v0.3, v0.2.1, v0.2, v0.1 and the
