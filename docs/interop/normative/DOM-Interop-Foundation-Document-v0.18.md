...
> **SUPERSEDED** — this version was superseded by
> `DOM-Interop-Foundation-Document-v0.19.md`, which records D-029
> (Relay V1 message-kind registry amended to admit
> `0x0005 = RouteTransportV1`, ratified by the operator on 2026-08-19).
> Kept as history only and must not be used as context authority.
...
State:               PARTIALLY RATIFIED — D-000, D-001, D-005, D-006, D-007,
                     ...
                     D-023, D-024, D-025, D-026, D-027, D-028 and A7
                     ratified; the rest
...
Product name:        DECIDED (D-009): no standalone product name. This is
                     ...
                     component name
Supersedes:          v0.17, v0.16, v0.15, v0.14, v0.13, v0.12, v0.11, v0.10, v0.9, v0.8, v0.7, v0.6, v0.5, v0.4, v0.3, v0.2.1, v0.2, v0.1 and the
...
Gate status:         G-F0 = PASS (docs/reports/F0-CLOSURE.md, waiver
                     ...
                     G-F5 = PASS; F5 = COMPLETE (D-012/D-027;
                     docs/reports/F5-CUSTOM-SIGNET-E2E.md). The gate was
                     closed on regtest plus custom Signet only; under
                     D-027 Public Signet is optional and non-gating.
                     G-F6 = PASS; F6 = COMPLETED
                     (docs/reports/F6-CLOSURE.md §7; D-028, operator
                     adjudication 2026-08-12) — the four §6 conditions
                     verified at main 56a0d06 and the gate's three
                     clauses re-run green.
                     NEXT REQUIRED GATE = G-F7, which is BLOCKED BY
                     EXTERNAL DEPENDENCY: the DOM-side Scriptless
                     Phases 2-6 are not this project's deliverable, and
                     no F7 work closes until they land. G-F8 NOT
                     STARTED.
```
...
(HEAD 9c04d36) remains on the record as history, per D-024. At that
2026-08-11 adjudication, the next required gate became G-F5; its subsequent
2026-08-12 closure is recorded below.

### F5 — Bitcoin leg
Execution specification: `docs/normative/DOM-Interop-Annex-M-v3.3-Bitcoin-Leg.md`
...
Gate G-F5 (Annex M M.15.2): DOM(dom-sim)↔BTC E2E on regtest and custom Signet,
...
State (2026-08-12, D-027 closure evidence): **G-F5 = PASS; F5 = COMPLETE.**
The content-addressed execution and independent audit record are
`docs/reports/F5-CUSTOM-SIGNET-E2E.md` and
`docs/reports/F5-INDEPENDENT-AUDIT.md`. Public Signet did not participate
in the gate and retains no gate, roadmap or release authority.
...
State (operator, 2026-08-12, D-028): **G-F6 = PASS; F6 = COMPLETED.**
The deferral recorded on 2026-08-10 is lifted: its stated cause — G-F3
and G-F4 pending and G-F5 awaiting its network leg — was removed by
D-025, D-026 and D-027, so the §8 ordering withholds nothing. The four
conditions that deferral attached to a no-re-run adjudication were each
verified at main `56a0d067bb668be2d21afbc1dbc2607532367ce2`: the eleven
evidence commits are all ancestors of main; `crates/f6-engine`,
`crates/relay`, `crates/rfq` and `crates/f6-model` are byte-identical to
the F6 baseline, as are `kaystra-core`, `store`, `uspe`, `btc-crypto`,
`dom-sim`, `counterparty-api` and `f2-harness`; the pin
`eb6aa1ca59226bc316e3aace5ee0e279e5a154c2` and every `dom-*` lockfile
source line are unchanged; and `./scripts/ci_local.sh` exits 0 at that
head. The gate's three clauses were re-run green anyway. Closure report:
`docs/reports/F6-CLOSURE.md` §7. **NEXT REQUIRED GATE = G-F7**, which is
...
A8  RESOLVED — formal DOM-Schnorr ↔ BIP340 bridge, incl. x-only
    ...
    Annex M v3.3 adopted).
A9  RESOLVED — testnets: EVM = Sepolia (F3); BTC networks = D-014
    as superseded by D-027 RATIFIED 2026-08-12: regtest + custom Signet
    BIP-325 are mandatory for F5; Public Signet is optional and outside
...
D-014  2026-08-10  SUPERSEDED by D-027 (historical record)
  ...
  Status:        SUPERSEDED in full by D-027 on 2026-08-12. Preserved
                 unchanged above as historical evidence only; it has no
                 active gate, roadmap or release force.
...
D-026  2026-08-11  RATIFIED (explicit operator adjudication) — G-F4 = PASS
  ...
  Effects on the gate sequence:
                 At the time of D-026, G-F0, G-F1, G-F2, G-F3 and G-F4
                 were closed and G-F5 became the next required gate.
                 The original Public-Signet characterization in this D-026
                 effect is superseded by D-027. G-F6
                 stays deferred — of the three gates its own closure
                 names as blocking, G-F3 and G-F4 are now closed and
                 G-F5 was then outstanding. This decision does not promote,
...
D-027  2026-08-12  RATIFIED (explicit sovereign operator decision)
  Problem:       D-014 made Bitcoin Public Signet an active F5
                 requirement even though its external faucet, propagation
                 and block cadence cannot be made reproducible or
                 operator-controlled. That requirement cannot govern a
                 deterministic engineering gate.
  Decision:      F5 requires exactly regtest plus a fast custom Signet
                 under BIP-325. The custom Signet uses an official Bitcoin
                 Core release without fork or mock, a non-trivial P2PK
                 1-of-1 challenge, an ephemeral signing key outside Git,
                 the official Core Signet miner and distinct miner/signer
                 and observer processes connected over P2P. Bitcoin Public
                 Signet is optional, outside the F5 gate, roadmap and
                 release, and can never block F5, F6, F7 or F8. Mainnet
                 remains excluded.
  Conformance:   the short CSV profile is committed in
                 infra/signet/conformance-terms.json and applies only to
                 conformance. The production CSV profile is not reduced.
                 The identity, challenge, challenge hash, magic, Core
                 version, ports, peers and confirmation policy are frozen
                 in infra/signet/network.json and credential-free configs.
  Gate effect:   M.15.2 removes only the Public-Signet gate operand. Public-Signet
                 scripts remain an optional interoperability utility and
                 confer no gate, roadmap or release status. No failure or
                 non-execution on Public Signet changes an F5-F8 result.
  Evidence:      scripts/f5-signet-custom-e2e.sh; infra/signet/; the
                 versioned execution report and content-addressed evidence
                 manifest produced by the closing run.
  Impact:        A9, F5, Annex M v3.3, G-F5, progress report, runbooks,
                 workflow, guards and evidence. No Bitcoin consensus,
                 DOM core, DOM wire, DOM mempool, DOM Wallet or
                 dom-adaptor primitive changes are authorized.
  Supersedes:    D-014 in full. D-014 remains in this registry marked
                 SUPERSEDED and has historical force only.
  Status:        RATIFIED by the operator's explicit instruction on
                 2026-08-12. This decision is authoritative immediately;
                 the technical gate still requires every revised M.15.2
                 proof to be green before F5 COMPLETE / G-F5 PASS.

D-028  2026-08-12  RATIFIED (explicit operator adjudication) — G-F6 = PASS
  Object:        formal adjudication of gate G-F6 (§7, "F6 — RFQ, solver
                 and Relay") and the completion of phase F6.
  Decision:      G-F6 = PASS. F6 = COMPLETED. NEXT REQUIRED GATE =
                 G-F7. The deferral of 2026-08-10 is lifted. No document
                 in force may carry "EVIDENCE COMPLETE", "adjudication
                 deferred", "pending" or "partial" as this gate's state.
  Cause of the deferral, removed:
                 the 2026-08-10 record deferred adjudication for one
                 stated reason — the §8 ordering, with G-F3 and G-F4
                 pending and G-F5 awaiting its network leg, and no prior
                 written waiver. D-025 closed G-F3, D-026 closed G-F4
                 and D-027 closed G-F5. The ordering now withholds
                 nothing, and no waiver was invented to reach this
                 result.
  Basis:         §6 of docs/reports/F6-CLOSURE.md permits adjudication
                 WITHOUT re-running the tests under four conditions.
                 Each was verified against origin/main at
                 56a0d067bb668be2d21afbc1dbc2607532367ce2, by
                 measurement and not by reading the earlier record:
                 (1) the eleven evidence commits — 6e6ea40, 04ec3fd,
                 0bc92f4, 5c62151, d65be31, bae94a4, 7f8703c, e791496,
                 d831543, a37b49b, 27f7a37 — all exist and are
                 ancestors of main; (2) `git diff 27f7a37..origin/main`
                 over crates/f6-engine, crates/relay, crates/rfq and
                 crates/f6-model is EMPTY, as it is over kaystra-core,
                 store, uspe, btc-crypto, dom-sim, counterparty-api and
                 f2-harness; (3) DOM_ADAPTOR_REV
                 eb6aa1ca59226bc316e3aace5ee0e279e5a154c2 is identical
                 and every dom-* source line in Cargo.lock is identical
                 by direct diff; (4) ./scripts/ci_local.sh exits 0 at
                 that head in 336 s with verdict PASS.
  The two changes on the consumed surface, examined not waved past:
                 crates/f4-harness/tests/e2e_anvil.rs changed by 226
                 lines, but it is that crate's TEST BINARY —
                 crates/f4-harness/src and its manifest are untouched
                 and f6-engine links the library, never another crate's
                 test targets, so it cannot reach F6. Cargo.lock gained
                 10 lines, which are f5-e2e acquiring dependencies among
                 them f6-engine, rfq, uspe and adapter-dom-sim: a NEW
                 CONSUMER of F6, not a change to it, and f6-engine,
                 relay and rfq keep exactly their prior dependency sets.
  Re-run evidence (not required by §6, performed anyway):
                 the gate's three clauses were measured green at the
                 adjudicated head — complete settlement with a solver,
                 g_f6_e2e.rs 5/0; total loss of the Relay and its
                 database, g_f6_relay_loss.rs 4/0; ACK, dedup and
                 byte-identical retransmission, relay_transport.rs 8/0
                 with d019_message_type_policy.rs 18/0. Also green: the
                 D-023 carry, d023_terms_carry.rs 7/0; the D-019
                 recipient payload check, d019_consumer_payload.rs 7/0;
                 the Relay library 7/0 and rfq 19/0. f6-model drives the
                 REAL rfq::selection rather than a re-model and proves
                 all fifteen named refusals reachable, none vacuous.
  Scope:         this decision closes G-F6 and F6 and promotes nothing
                 else. G-F7 stays BLOCKED BY EXTERNAL DEPENDENCY — the
                 DOM-side Scriptless Phases 2-6 are outside this
                 project — and G-F8 stays NOT STARTED. No F6 code,
                 contract, script, workflow, vector or manifest was
                 modified in order to adjudicate, and nothing was
                 executed on any chain for it.
  Recorded debt, not converted into PASS:
                 no independent external audit has covered F6. Annex M
                 M.18's six-role independent audit governs F5 and was
                 not met there either — that record declares five review
                 passes and "no claim of an external third party" — and
                 F6 never had one at all. External composition audit is
                 F8's deliverable. The absence is recorded so it stays
                 visible instead of dissolving with time.
  Components:    docs/reports/F6-CLOSURE.md §7; this registry; §7 F6
                 state; the Gate status header.
  Status:        RATIFIED. The operator evaluated and adjudicated on
                 2026-08-12; the executor verified the four conditions,
                 re-ran the clauses and records the decision.
```
...
*This v0.18 supersedes v0.17, v0.16, v0.15, v0.14, v0.13, v0.12, v0.11, v0.10, v0.9, v0.8, v0.7, v0.6, v0.5, v0.4, v0.3, v0.2.1, v0.2, v0.1 and the
...
ratified. The single change of this version over v0.17 is D-028: the
operator's adjudication of G-F6 = PASS and F6 = COMPLETED, with the gate
status header and §7 F6 updated to match and G-F7 named as the next
required gate — G-F7 being BLOCKED BY EXTERNAL DEPENDENCY. No other
decision, invariant, phase, open question or pin was altered, and no code
was touched to reach this adjudication.*
