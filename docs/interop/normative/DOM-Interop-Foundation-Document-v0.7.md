...
> # SUPERSEDED
>
> **This version is no longer the context authority and leaves agent
> circulation (§12.2).** It was superseded on 2026-08-10 by
> **v0.8**, which carries the ratified decision **D-017** (adoption of the
> F4 engineering specification as the normative execution authority for
> the Minimal USPE). Kept as history only. The current authority is
> `DOM-Interop-Foundation-Document-v0.8.md`.
...
State:               PARTIALLY RATIFIED — D-000, D-001, D-005, D-006, D-007,
                     D-008, D-009, D-010, D-011, D-012, D-016 and A7
                     ratified; D-013/D-014/D-015 RATIFICATION PENDING (F5
                     conformance evidence); the rest remains DRAFT (/goat
...
Product name:        DECIDED (D-009): no standalone product name. This is
                     ...
                     component name
Supersedes:          v0.6, v0.5, v0.4, v0.3, v0.2.1, v0.2, v0.1 and the
...
Gate status:         G-F0 = PASS (docs/reports/F0-CLOSURE.md, waiver
                     ...
                     (docs/reports/F2-CLOSURE.md). F5 IN PROGRESS in
                     parallel (D-012); execution spec is Annex M v3.2.
```
...
A8  Formal DOM-Schnorr ↔ BIP340 bridge, incl. x-only parity (F5) —
    execution spec adopted (Annex M v3.2, D-012); the bridge decision
    is D-013, RATIFICATION PENDING until C1a/C1b/C2/C3/C4 are green.
A9  Chosen testnets — EVM (F3); BTC networks are D-014 (regtest +
    custom signet + public signet, mainnet excluded), RATIFICATION
    PENDING until the F5 execution evidence exists.
...
D-013  2026-08-10  RATIFICATION PENDING (registered, not ratified)
  ...
  Decision:      adopt Annex M v3.2 AFTER C1a/C1b/C2/C3/C4 are green;
                 pin secp256k1-zkp at the recorded revision; key-path
                 SIGHASH_DEFAULT and a single-leaf CSV refund. Backend
                 pinned in crates/adapters/btc-secp-sys (upstream rev
  ...
  Status:        RATIFICATION PENDING until the C1–C4 evidence exists;
                 the executor prepares the record and does not self-
                 ratify. Supersedes A8 upon a favorable decision.

D-014  2026-08-10  RATIFICATION PENDING (registered, not ratified)
  ...
  Decision:      regtest + custom signet + public signet for F5;
                 mainnet excluded.
  Status:        RATIFICATION PENDING until the F5 execution evidence
                 (configs, genesis, challenge, versions, runbooks,
                 results) exists. Supersedes the BTC half of A9 upon a
                 favorable decision.
...
*This v0.7 supersedes v0.6, v0.5, v0.4, v0.3, v0.2.1, v0.2, v0.1 and the
