...
| Secret two-nonce derivation | ratified NAR-001 section 7; ADR-0019 | OS-CSPRNG derivation, exact three tags, checked pre-export retry, authoritative wide reduction, and RAII zeroization exist only behind the non-default evidence feature pending G1b integration | deterministic KDF separation tests, default-feature quarantine checks, and 10,000 real-verifier cycles | signed KAT inputs only; independent intermediate outputs pending | evidence implementation complete; production API deliberately unavailable; gate open |
| One-shot partial signing | ratified NAR-001 sections 7.4 and 8; ADR-0019 | opaque pre-authorization pair, exact durable permit binding, authorized public export, consuming partial sign, bound verification, and aggregation are quarantined from default builds | two-participant closed workflow through unchanged DOM verifier; default production compile excludes raw capability APIs | independent aggregate vector and G1b conformance pending | evidence implementation complete; production integration blocked on G1b |
...
| Nonce derivation API safety | ratified NAR-001 section 7 and NAR-002 section 16; ADR-0019/0020 | default production builds omit raw derivation, public nonce, and partial-signing APIs; the complete one-shot flow is evidence-only under explicit `test-helpers` | default no-feature check/clippy, one-shot lifecycle tests, and nonce-derivation fuzz target | independent audit identified the bypass and required fail-closed quarantine | **code/integration blocker:** helper-only evidence cannot serve production; lower boundary must authenticate G1b capability before exposure/signing |
