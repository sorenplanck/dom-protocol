...
# `R̂ = R + U` with something this project invented.
#
...
module="crates/dom-scriptless-crypto/src/refund_adaptor_round.rs"
suite="crates/dom-scriptless-crypto/tests/refund_adaptor_round_v1.rs"
crate_src="crates/dom-scriptless-crypto/src"
self_path="scripts/check-refund-adaptor-two-nonce.sh"

capability="CompletedRefundAdaptorCycleV1"
...
if grep -Eq 'aggregate_public_nonces_v1\(&\[[[:space:]]*aggregate_nonce\.clone\(\),[[:space:]]*inputs\.refund_adaptor_point\.clone\(\),?[[:space:]]*\]\)' <<<"$crate_flat"; then
