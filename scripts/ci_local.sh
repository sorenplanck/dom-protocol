...
run_static() {
  ...
  run_gate "Python guard unit tests" run_python_guard_tests
  run_gate "new shell syntax" check_new_shell_syntax
  run_gate "new shell lint" \
    shellcheck "$script_dir/guards.sh" "$script_dir/ci_local.sh" \
      "$script_dir/check-authorship.sh"
  run_gate "absorbed interoperability guards" "$script_dir/guards.sh"

  run_gate "workspace boundaries" "$script_dir/check-boundaries.sh"
  run_gate "two-nonce adaptor boundary" "$script_dir/check-adaptor-two-nonce.sh"
  run_gate "claim adaptor provenance" "$script_dir/check-claim-adaptor-provenance.sh"
  run_gate "hash-domain freeze" "$script_dir/check-hash-domains.sh"
  run_gate "normative adjudication" "$script_dir/check-normative-adjudication.sh"
  run_gate "policy topology" "$script_dir/check-policy-topology.sh"
  run_gate "shared-output Bulletproof boundary" "$script_dir/check-shared-output-bp.sh"
  run_gate "ratification signatures" "$script_dir/check-ratification-signatures.sh"
  run_gate "mainnetswap publication boundary" check_publication_boundary

  run_gate "absorption manifest" python3 -B "$script_dir/check-absorption-manifest.py"
  run_gate "production-readiness manifest" \
    python3 -B "$script_dir/check-production-readiness-manifest.py"
...
run_offline() {
  ...
  run_gate "workspace formatting" cargo fmt --all -- --check
  run_gate "layer check" \
    cargo check --locked --offline "${package_args[@]}" --all-targets
  run_gate "layer clippy" \
    cargo clippy --locked --offline "${package_args[@]}" --all-targets -- -D warnings
  run_gate "layer tests" \
    cargo test --locked --offline "${package_args[@]}" --all-targets
  run_gate "layer documentation tests" \
    cargo test --locked --offline "${package_args[@]}" --doc

  run_gate "real DOM adaptor backend" \
    cargo test --locked --offline --package dom-leg \
      --features real-dom-adaptor --all-targets
  run_gate "real EVM HTTP backend" \
    cargo test --locked --offline --package adapter-evm \
      --features rpc-http --all-targets
  run_gate "production daemon check" \
    cargo check --locked --offline --package dom-interopd \
      --no-default-features --features production --lib --bins
  run_gate "production daemon clippy" \
    cargo clippy --locked --offline --package dom-interopd \
      --no-default-features --features production --lib --bins -- -D warnings
  run_gate "production daemon tests" \
    cargo test --locked --offline --package dom-interopd \
      --no-default-features --features production --lib --bins
  run_gate "Store release-surface refusal" "$script_dir/check-release-surface.sh"
  run_gate "Relay release-surface refusal" "$script_dir/check-relay-fault-surface.sh"
...
run_live_local() {
  ...
  run_gate "EVM contract release" "$repo/contracts/scripts/check_release.sh"
  run_gate "local Anvil E2E" "$script_dir/e2e_anvil.sh"
  run_gate "local Bitcoin Core Regtest E2E" "$script_dir/f5-regtest-e2e.sh"
  ...
  case "$mode" in
    --static)
      if [[ "${DOM_CI_STATIC_SANDBOX:-0}" == "1" ]]; then
        ...
        current_namespace="$(readlink /proc/self/ns/net 2>/dev/null)"
        ...
        local mount_options
        mount_options="$(findmnt -T "$repo" -n -o OPTIONS 2>/dev/null)"
      ...
      parent_namespace="$(readlink /proc/self/ns/net 2>/dev/null)"
