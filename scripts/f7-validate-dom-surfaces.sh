...
# RESTORED: the node repaired its migration lab (the oracle now embeds
# `bulletproof_bp.rs` via include_str! instead of naming the nonexistent
# `dom_crypto::bulletproof` module), so the deferred coverage returns, as the
# `the_node_owned_migration_lab_is_still_broken` canary in f2-harness
# instructed the day this happened. That canary is deleted in the same change.
