...
//!
//! Moved verbatim from `dom-wallet` (v1) `src/rpc_client.rs` as part of the
//! wallet-migration Phase D re-homing (WALLET_V2_DESIGN §7.3): the desktop is
//! its only remaining consumer. Wire contract unchanged. The only deviation
//! from the v1 file: the `impl From<RpcClientError> for WalletError` (a v1
//! ergonomics shim) was dropped along with its `crate::types` import — the
//! desktop never converts into v1's `WalletError`.
//!
...
//!
//! The client is decoupled from wallet state. RPC failures
