...
//!
//! Moved verbatim from `dom-wallet` (v1) `src/registry.rs` as part of the
//! wallet-migration Phase D re-homing (WALLET_V2_DESIGN §7.3): the desktop is
//! the registry's only consumer, so the code now lives here. The on-disk
//! `registry.json` format is unchanged (`REGISTRY_FORMAT_V1`); v1's frozen copy
//! remains in the deprecated crate but is no longer used by the desktop.
//!
...
//! - `vault_path` — where the encrypted wallet directory (`WalletDir`) lives.
